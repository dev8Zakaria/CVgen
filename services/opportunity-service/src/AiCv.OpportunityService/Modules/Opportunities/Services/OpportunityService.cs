...
namespace AiCv.OpportunityService.Modules.Opportunities.Services;
...
public class OpportunityService
{
    ...
    {
        var userId = await _repository.GetOrCreateUserIdByKeycloakIdAsync(keycloakId);
    ...
    {
        var userId = await _repository.GetOrCreateUserIdByKeycloakIdAsync(keycloakId);
        var jobOffers = await _repository.GetAllByUserIdAsync(userId);
    ...
    {
        var userId = await _repository.GetOrCreateUserIdByKeycloakIdAsync(keycloakId);
        var jobOffer = await _repository.GetByIdAndUserIdAsync(id, userId);
